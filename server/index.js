...
// --------------------------------------------------------------
// Set db below!
// Options: sql, postgres, cassandra
...
if (database === 'sql') {
  ...
  const dbConnection = require('./database/indexC.js');
  const { Stock, Products, Stores } = require('./database/modelsCassandra.js');

  app.get('/products/:productId', (req, res) => {
    Products.find({id: parseInt(req.params.productId)}, (err, product) => {
      if (err) {
        res.send(err);
      } else {
        console.log(product);
  ...
  app.get('/stores/:storeId', (req, res) => {
    Stores.find({id: parseInt(req.params.storeId)}, (err, store) => {
      if (err) {
        res.send(err);
      } else {
        res.send(store);
  ...
  app.get('/stock/:productId', (req, res) => {
    Stock.find({product_id: parseInt(req.params.productId)}, (err, products) => {
      if (err) {
        res.send(err);
      } else {
        res.send(products);
